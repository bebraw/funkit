...
suite(f.partial(f.rtrim, ' r'), [
    'foobar', 'fooba',
    'foobar ', 'fooba',
    'batbat', 'batbat'
]);

suite(f.partial(f.trim, ' f'), [
    'foobar', 'oobar',
    ' foobar ', 'oobar',
    'batbat', 'batbat'
]);

suite(f.partial(f.partition, ' '), [
...
suite(f.partial(f.separate, 'foo<a>barbar</a>zoo'), [
    ['<a>', '</a>'], 'barbar',
    ['foo', 'zoo'], '<a>barbar</a>',
    ['moo', 'moo'], ''
]);
...
suite(f.partial(f.equals, [{name: 'rofl'}, {name: 'lol'}]), [
...
suite(suite.async(f.partial(f.parallel, getName)), [
    [['1', '2', '3']], ['1', '2', '3']
]);
